...
class TestGitFlowRelease(unittest.TestCase):
    ...
    def __release_start(self):
        GitFlow(self.state_handler).with_branch(Branches.RELEASE).set_action(Actions.START).process()

    def __release_finish(self):
        GitFlow(self.state_handler).with_branch(Branches.RELEASE).set_action(Actions.FINISH).process()
